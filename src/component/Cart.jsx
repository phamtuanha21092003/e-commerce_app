...
export function Cart() {
    ...
    const totalOrder = productCheckout.reduce((accumulator, currentElement) => {
        const { count, price } = currentElement
        return parseFloat(count) * parseFloat(price) + accumulator
    }, 0)
    const countOrder = productCheckout.reduce((accumulator, currentElement) => {
        ...
        <div className="cart_container" id="cart_modal">
                ...
                <div className="cart_header">
                    <div className="product_checkbox">
                        <input
                            ...
                            onChange={(event) =>
                                setChecked(
                                    checked.map(() => event.target.checked)
                                )
                            }
                        />
            ...
            {!isOrdering && (
                <div className="cart_price">
                    ...
                    <div>
                        Tổng thanh toán ({countOrder} Sản phẩm): {totalOrder} $
                    </div>
                    <div className="sell_wrapper" onClick={ordering}>
